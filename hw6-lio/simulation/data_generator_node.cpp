...
int main(int argc, char **argv)
{
    ...
    ros::Publisher pub_feature = n.advertise<sensor_msgs::PointCloud>("image", 1000);
    ...
    ros::Publisher pub_lidar_cloud = n.advertise<sensor_msgs::PointCloud>("lidar_cloud", 1000);
    ...
    while (ros::ok())
    {
        ...
        if (publish_count % generator.IMU_PER_LIDAR == 0) {
            sensor_msgs::PointCloud point_cloud;
            point_cloud.header.frame_id = "world";
            point_cloud.header.stamp = ros::Time(current_time);
            for (auto &it: generator.getLidarCloud()) {
                geometry_msgs::Point32 p;
                p.x = it(0);
                p.y = it(1);
                p.z = it(2);
            ...
            pub_lidar_cloud.publish(point_cloud);
        }
