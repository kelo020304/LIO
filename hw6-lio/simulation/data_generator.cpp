...
Vector3d DataGenerator::getPosition()
{
    ...
    return Vector3d(x, y, z) - Vector3d(10, 10, 10);
}
...
vector<Vector3d> DataGenerator::getLidarCloud()
{
    Vector3d position = getPosition();
    Matrix3d quat = getRotation();
    ...
    for (int i = 0; i < NUM_POINTS; i++)
    {
        ...
        double yy = pts[i * 3 + 1] - position(1);
        double zz = pts[i * 3 + 2] - position(2);
        Vector3d local_point = quat.inverse() * Vector3d(xx, yy, zz);
        xx = local_point.x();
        yy = local_point.y();
        zz = local_point.z();
        if(zz > 0.0 && std::fabs(atan2(xx, zz)) <= M_PI * FOV / 2 / 180 && std::fabs(atan2(yy, zz)) <= M_PI * FOV / 2 / 180) {
            cloud.push_back(local_point);
        }
    }
    return cloud;
}
